...
pair<TFile*, pair<TH3D*, TH3D*>> getOpticalMap(const char * mapDir){
    TFile *mapFile;
    TH3D *hMap;
    // TH2D *oMap;
    TH3D *oMap;    // the old map is always 3d
    ...
    pair<TH3D*, TH3D*> map_pair = make_pair(hMap, oMap);	// old map use 3d
    // pair<TFile*, pair<TH3D*, TH2D*>> filemap_pair = make_pair(mapFile, map_pair);	//old map use 3d
    pair<TFile*, pair<TH3D*, TH3D*>> filemap_pair = make_pair(mapFile, map_pair);
    return filemap_pair;
}
...
// Note: to use old map, I use 2 3d maps -> in future correct with 2d
void addBranches(TTree * reducedTree, TH3D * hMap, TH3D * oMap, Int_t event_x_file, Int_t file_number){
	...
	// Use interior/exterior map, based on coordinate
        if (r >= 300){      // WAIT: exterior map is bugged 
            ...
            Int_t bin = oMap->FindBin(x, y, z);		//Old map use coordinate
...
void data_cleaning(const char * dirIn, const char * dirOut, const char * mapDir){
    ...
    pair<TFile*, pair<TH3D*, TH3D*>> pair_file_map = getOpticalMap(mapDir);	// the old map use 3d
    TFile* mapFile = pair_file_map.first;
    // pair<TH3D*, TH2D*> map_pair = pair_file_map.second;
    pair<TH3D*, TH3D*> map_pair = pair_file_map.second;    // old map -> use 3d maps
    TH3D* hMap = map_pair.first;
    // TH2D* oMap = map_pair.second;
    TH3D* oMap = map_pair.second;    // old map-> use 3d
