...
pair<Double_t, Double_t> getHitOnGermanium(Double_t x1, Double_t y1, Double_t x2, Double_t y2){
...
void runToyOpticsFromPoint(Double_t x1, Double_t y1, Int_t nOptics, TH1D * &prInnerD, TH2D* &innerMap, TH2D* &outerMap){
	...
	while(phi < 2*PI){
		...
		if(pointPlacement==INNER_REGION){
			...
			ge_int = getHitOnGermanium(x1, y1, x2, y2);
