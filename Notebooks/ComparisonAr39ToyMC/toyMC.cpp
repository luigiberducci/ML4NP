...
const char* mapFile = "OpticalMapL200XeD.14String.5mm.root";
...
TH1D* getPhotonNrDist(uint32_t events, double radius, double halfHeight, const std::string& histName = ""){
	...
	TH1D* distr = new TH1D(name, name, 1500, 0., 1499.);
...
double getSum(){
	...
	TH3D* map_scaled = dynamic_cast<TH3D*>(file->Get("ProbMapInterior"));
